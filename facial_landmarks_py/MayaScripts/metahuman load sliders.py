    ...
    def loadTxt(*args):
        ...
        for i in range(0, len(selected)):
            ...
            for i in range(len(t)):
                cmds.setKeyframe(node+"."+attr, v=v[i], t=t[i]*30)
    return loadTxt
...
cmds.textField(path, edit=True, enterCommand=textLoading(path))

# button for loading the motion file
cmds.button( label='loadMotion', command=textLoading(path))
cmds.button( label='clearCurves', command=clearSelectedNodes)
cmds.showWindow()